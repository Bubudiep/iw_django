                                ...
                                if qs_space is not None and is_takeaway==False:
                                    ...
                                    if qs_space.is_inuse==True and qs_space.user_use==user:
                                        ...
                                        cr_oder=Restaurant_order.objects.filter(restaurant=qs_restaurant,
                                                                                    user_order=user,
                                                                                    space=qs_space).first()
                                        if cr_oder.status=="COMPLETE" or cr_oder.status=="DELIVERED":
...
class NhanvienSortingViewSet(viewsets.ModelViewSet):
    serializer_class = NhanvienSortingSerializer
    authentication_classes = [OAuth2Authentication]
    permission_classes = [IsAuthenticated]
    pagination_class = StandardResultsSetPagination
    ...
    def get_queryset(self):
        return NhanvienSorting.objects.all()
